...
(async () => {
  ...
  const made = { audit_log: [], registration_requests: [], registration_appeals: [], whatsapp_conversations: [],
    whatsapp_messages: [], ussd_sessions: [], csp_reports: [], users: [] };
  ...
  try {
    ...
    for (const id of ['audit_log', 'rejected_registrations', 'whatsapp_conversations', 'ussd_sessions', 'csp_reports'])
    ...
    ok('each enforced policy left one audit entry saying what it removed', trail.length === 5, trail.map(t => t.entity_type));
    ...
    ok('a second run removes nothing and writes nothing', run2.policies.every(p => p.removed === 0 && (p.report_only || p.due === 0)) && trail2.length === 5);
    ...
    ok('overview.retention lists the six policies with their periods',
      ov.retention && ov.retention.policies.length === 6 && ov.retention.policies.every(p => p.keep && typeof p.due === 'number'), ov.retention);
    ...
    for (const t of ['whatsapp_messages', 'whatsapp_conversations', 'registration_appeals', 'registration_requests', 'ussd_sessions', 'csp_reports', 'audit_log', 'users']) await del(t);
