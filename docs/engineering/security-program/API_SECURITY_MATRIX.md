...
| POST | `/api/assist/assess` | JWT | any signed-in |  |
...
| PUT | `/api/auth/language` | JWT | any signed-in |  |
...
| POST | `/api/auth/recovery/complete` | — | public | Two-proof recovery, step 3 (D32). Needs the reset token. Password policy; ends every session; two-step sign-in unchanged; returns a new session. 10/network per 15 min. |
| POST | `/api/auth/recovery/start` | — | public | Two-proof recovery, step 1 (D32): username or email. 20/network and 5/identifier per 15 min. |
| POST | `/api/auth/recovery/verify` | — | public | Two-proof recovery, step 2 (D32): the flow id plus the codes it asks for (email, authenticator or recovery code). 30/network per 15 min. |
...
| PUT | `/api/auth/tour/:id` | JWT | any signed-in | Own account only; `:id` allow-listed, status and step validated, statuses never go backwards (D27). |
...
| POST | `/api/security/csp-report` | — | public | Browser CSP reports (D24): 16 KB cap, aggregated per site, never stored per request; foreign scripts become events (R8). |
...
24 endpoints take no session. Each one either needs a shared secret / capability
