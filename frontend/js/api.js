...
const API = (() => {
  ...
  return {
    ...
    // Trusted browsers (D34): the ones that skip the authenticator code at sign-in.
    ...
    getSecurityIncidents: (status = 'active') => request('GET', '/security/incidents?status=' + encodeURIComponent(status)),
    updateSecurityIncident: (id, body) => request('PATCH', `/security/incidents/${id}`, body),
    getIncidentEvidence: (id) => request('GET', `/security/incidents/${id}/evidence`),
    ...
    getSpares: (params = {}) => { const qs = new URLSearchParams(params).toString(); return request('GET', '/inventory/spares' + (qs ? '?' + qs : '')); },
    setSpare: (id, isSpare) => request('PATCH', `/inventory/${id}/spare`, { is_spare: isSpare }),
    swapDevice: (id, data) => request('POST', `/inventory/${id}/swap`, data),
    getSpares: (params = {}) => { const qs = new URLSearchParams(params).toString(); return request('GET', '/inventory/spares' + (qs ? '?' + qs : '')); },
    setSpare: (id, isSpare) => request('PATCH', `/inventory/${id}/spare`, { is_spare: isSpare }),
    swapDevice: (id, data) => request('POST', `/inventory/${id}/swap`, data),
    ...
    planVisit: (data) => request('POST', '/visits', data),
    updateVisit: (id, data) => request('PATCH', '/visits/' + id, data),
