...
const SecurityPage = (() => {
  ...
  const LAYERS = [
    ...
    { icon: 'ti-key', color: 'var(--purple)', title: 'Personal sign-in', status: 'in', evidence: 'code',
      ...
      tech: 'bcrypt hashes (cost 10–12). 20 attempts per account per 15 minutes, 120 per network.' },
    { icon: 'ti-clock-shield', color: 'var(--teal)', title: 'Sessions', status: 'partial', evidence: 'code',
      plain: 'After sign-in the browser holds a signed pass that expires after 7 days. The account is re-checked on every request, so suspending a person takes effect at once.',
      tech: 'Signed JWT, 7-day expiry, account status re-read per request. Gap: one pass cannot be cancelled on its own before it expires — suspend the account instead (SEC-005).' },
    { icon: 'ti-building-community', color: 'var(--green)', title: 'Roles and school isolation', status: 'in', evidence: 'tested',
      ...
      tech: 'Role check on every API router plus a per-record school check. 160 automated assertions (role matrix 125, security boundaries 35).' },
