...
const SecurityPage = (() => {
  ...
  const LAYERS = [
    ...
    { icon: 'ti-clock-shield', color: 'var(--teal)', title: 'Sessions', status: 'in', evidence: 'tested',
      plain: 'After sign-in the browser holds a signed pass that expires after 7 days. Changing a password signs out every other device; anyone can sign out everywhere from their profile menu; a suspended account stays signed out even when it is reactivated. The authenticator is asked on a new browser; a browser someone chose to trust needs only the password for 30 days (14 for head office), and signing out everywhere ends that too.',
