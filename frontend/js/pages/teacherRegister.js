const TeacherRegisterPage = (() => {
  ...
  function renderSubmitted() {
    ...
    <div class="reg-page">
      <div class="reg-card reg-card-status" style="max-width:520px;width:100%">
        ...
        <p class="reg-sub">Your registration is pending approval from your<br><strong>School Administrator</strong></p>
        <div class="reg-status-card">
          <div class="reg-status-row"><span>Status</span><span class="reg-badge pending">Pending Review</span></div>
          <div class="reg-status-row"><span>Email</span><span>${statusEmail || ''}</span></div>
        </div>
        <p class="reg-hint">This page will automatically update when your request is reviewed. You can close this page and check back later by logging in.</p>
        <div class="oe-footer-logo">${oeLogoSvg}</div>
      </div>
    </div>`;
  }
  ...
  function renderApproved() {
    ...
    <div class="reg-page">
      <div class="reg-card reg-card-status" style="max-width:520px;width:100%">
        <div class="oe-icon" style="margin:0 auto 16px">${oeIconSvg}</div>
        <h1 class="reg-title" style="color:var(--green)">Registration Approved!</h1>
        <p class="reg-sub">Welcome! Your account has been approved. You can now log in to access the system.</p>
        <button class="reg-btn" onclick="TeacherRegisterPage.goLogin()">
  ...
  function renderRejected() {
    ...
    <div class="reg-page">
      <div class="reg-card reg-card-status" style="max-width:520px;width:100%">
        ...
        ${rejectionReason ? `<div class="reg-status-card"><div class="reg-status-row"><span>Reason</span><span style="color:var(--text2)">${rejectionReason}</span></div></div>` : ''}
        <button class="reg-btn" onclick="TeacherRegisterPage.goLogin()">
  ...
  function showStatus(error, userId, email, reason) {
    ...
    if (error === 'teacher_pending') {
      step = 'submitted';
      startPolling();
    } else if (error === 'teacher_rejected') {
      step = 'rejected';
    }
  }
